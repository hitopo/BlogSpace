...
<section class="publicMian ">
    ...
    <div class="right">
        ...
        <form id="searchForm" name="searchForm" method="post"
              action="<c:url value="/actions/security/account/search"/> ">
            <div class="search">
                <span>查询条件：</span>
        ...
        <table class="providerTable" cellpadding="0" cellspacing="0">
            ...
            <c:forEach items="${accounts}" var="account">
                <tr>
                    ...
                    <td>
                        <a href="<c:url value="/actions/security/resetPassword/${account.id}"/> "><img
                                src="<c:url value="/img/mm.png"/>" alt="修改密码" title="修改密码"/></a>
                        <a href="<c:url value="/actions/security/updateAccount/${account.id}"/> "><img
                                src="<c:url value="/img/xiugai.png"/>" alt="修改" title="修改"/></a>
                        <a href="javascript:void(0)" onclick="removeAccount(${account.id})" class="removeBill"><img
                                src="<c:url value="/img/schu.png"/> " alt="删除" title="删除"/></a>
                    </td>
                </tr>
            </c:forEach>
            <tr>
...
<script>
    ...
    function removeAccount(id) {
        if (window.confirm("你真的要删除这个用户吗？")) {
            alert("删除成功！");
            window.location.href = "<c:url value="/actions/security/deleteAccount?accountId="/>" + id;
        }
    }
